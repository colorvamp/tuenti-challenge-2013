...
<?php
	...
	/* We dont need the origin key to decode other encoded messages, we just needed a encoded and a decoded
	 * string to decode other messages, so i choose the larger of thems as a source */
	$decoded = 'Some humans would do anything to see if it was possible to do it. If you put a large switch in some cave somewhere, with a sign on it saying \'End-of-the-World Switch. PLEASE DO NOT TOUCH\', the paint wouldn\'t even have time to dry.';
	$encoded = hex2str('');

	$i = file('php://stdin');
	$input = trim(array_shift($i));
	$input = hex2str($input);
	$final = array();for($i=0;$i<strlen($input);$i++){$final[$i] = $input[$i] ^ $decoded[$i] ^ $encoded[$i];}
	echo implode('',$final);
	exit;
	...
	$data[] = hex2str('');
	$data[] = hex2str('');
	...
	$GLOBALS['frecuency'] = array();
	$texts[] = 'Some humans would do anything to see if it was possible to do it. If you put a large switch in some cave somewhere, with a sign on it saying \'End-of-the-World Switch. PLEASE DO NOT TOUCH\', the paint wouldn\'t even have time to dry.';	
	//$GLOBALS['frecuency'] = json_decode('{" ":27,"e":26,"t":25,"a":24,"o":23,"h":21,"n":20,"s":19,"i":18,"r":17,"d":16,"l":15,"c":14,"u":13,"m":12,"w":11,"f":10,"g":9,"y":8,"p":7,"b":6,"v":5,"k":4,"j":3,"x":2,"q":1,"z":0}',1);
	$GLOBALS['frecuency'] = array(' '=>15,'.'=>13,'e'=>12.02,'E'=>12.02,'t'=>9.10,'T'=>9.10,'a'=>8.12,'A'=>8.12,'o'=>7.68,'O'=>7.68,','=>7.5,'i'=>7.32,'I'=>7.31,'n'=>6.96,'N'=>6.95,'s'=>6.29,'S'=>6.28,'h'=>6.03,'H'=>6.02,'r'=>5.93,'R'=>5.92,'d'=>4.33,'D'=>4.32,'l'=>3.99,'L'=>3.98,'c'=>2.89,'C'=>2.88,'u'=>2.72,'U'=>2.71,'m'=>2.62,'M'=>2.61,'w'=>2.31,'W'=>2.30,'f'=>2.12,'F'=>2.11,'g'=>2.10,'G'=>2.09,'y'=>2.04,'Y'=>2.03,'p'=>1.83,'P'=>1.82,'b'=>1.50,'B'=>1.49,'v'=>1.12,'V'=>1.11,'k'=>0.70,'K'=>0.69,'j'=>0.18,'J'=>0.17,'x'=>0.12,'X'=>0.11,'q'=>0.11,'Q'=>0.10,'z'=>0.08,'Z'=>0.07);
	$key = getPossibleKey($data);
	//echo $key;

	function getPossibleKey($streams){
		$maxLen = 0;foreach($streams as $stream){$l = strlen($stream);if($l > $maxLen){$maxLen = $l;}}
		$minLen = strlen(end($streams));//foreach($streams as $stream){$l = strlen($stream);if($l < $minLen){$minLen = $l;}}
		$cleanKey = array();for($keyPos = 0;$keyPos < $minLen;$keyPos++){
			$prob = -1;$maxFreq = 0;
			for($i=0;$i<255;$i++){//ascii charset
				$freq = 0;foreach($streams as $stream){
					if(!isset($stream[$keyPos])){continue;}
					$char = $stream[$keyPos];
					$res = $i ^ ord($char);
					$ascii = chr($res);
					if(isset($GLOBALS['frecuency'][$ascii])){$freq += $GLOBALS['frecuency'][$ascii];}
					if($freq > $maxFreq){$maxFreq = $freq;$prob = $res;}
				}
			}
			$cleanKey[$keyPos] = $prob;
		}
		$cleanKey = array_map(function($n){return chr($n);},$cleanKey);
		return implode('',$cleanKey);
	}

	function hex2str($hex){$str = '';for($i=0;$i<strlen($hex);$i+=2){$str .= chr(hexdec(substr($hex,$i,2)));}return $str;}
	function xorStrings($str1,$str2){$chars1 = str_split($str1);$chars2 = str_split($str2);$final = '';foreach($chars1 as $k=>$v){$final[$k] = chr(ord($chars1[$k]) ^ ord($chars2[$k]));}return implode('',$final);}
?>
